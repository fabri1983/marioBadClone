...
public class GoalKeyAction : MonoBehaviour {
	
	public EnumGoalKeyAction activation = EnumGoalKeyAction.ACTIVATION_UP;
	...
	public bool isActivationValid () {
		if (triggerA != -1 && triggerB != -1) {
			switch (activation) {
				case EnumGoalKeyAction.ACTIVATION_UP:
					return Gamepad.Instance.isUp();
				case EnumGoalKeyAction.ACTIVATION_DOWN:
					return Gamepad.Instance.isDown();
				case EnumGoalKeyAction.ACTIVATION_RIGHT:
					return Gamepad.Instance.isRight();
				case EnumGoalKeyAction.ACTIVATION_LEFT:
	...
	public static bool beginCollisionWithPlayer (ChipmunkArbiter arbiter) {
		...
		shape1.getOwnComponent<GoalKeyAction>().flagTargetInside(shape1._handle.ToInt32());
	...
	public static void endCollisionWithPlayer (ChipmunkArbiter arbiter) {
		...
		shape1.getOwnComponent<GoalKeyAction>().flagTargetOutside(shape1._handle.ToInt32());
	}
}
