...
public class AnimateTiledTexture : MonoBehaviour
{
	...
	public float _framesPerSecond = 1f;				// Frames per second that you want to texture to play at
	public bool _pingPongAnim = false;				// True for going forward and backwards in the animation
	public int _rowsTotal = 1;						// How much rows the sprite has. This value is defined once
	public int _maxColsInRows = 1;					// How much columns the sprite has. This can be changed depending what row/s you want to animate
	...
	public Vector2 _scale = new Vector2(1f, 1f);	// scale the texture. This must be a non-zero number. negative scale flips the image.
	...
	public int[] _rowLimits = new int[]{0,1};		// start row and number of rows for current animation (0-based)
	public int[] _colLimits = new int[]{0,1};		// start column and number of sprite frames for current animation (0-based)
	...
	private void Awake()
    {
		...
		offsetYStart = 1f - 1f / (_rowsTotal - _rowLimits[0]);
    }
	...
	public void setRowLimits(int start, int numRows) {
		...
		offsetYStart = 1f - 1f / (_rowsTotal - start);
	}
    ...
    private void ApplyOffset()
    {
        ...
        Vector2 offset = new Vector2(
			...
			1f - ((_index / _maxColsInRows) / (float)_rowsTotal) - offsetYStart //y index
		);
        ...
        offset.y += ((1f / _rowsTotal) - _textureSize.y) / 2.0f;
